...
def get_args_parser():
    ...
    parser.add_argument('--use_qmcmf', default=False, action='store_true',
                        help="Use Q-MCMF matcher instead of Hungarian")
    parser.add_argument('--qmcmf_iou_thresh', default=0.5, type=float,
                        help="IoU threshold for Q-MCMF edge pruning (new classes)")
    parser.add_argument('--qmcmf_pseudo_iou_thresh', default=0.7, type=float,
                        help="IoU threshold for Q-MCMF edge pruning (old/pseudo classes)")
    ...
    parser.add_argument('--base_cls', default=70, type=int,
                        help='Number of classes in phase 0')
    parser.add_argument('--cls_per_phase', default=10, type=int)
    parser.add_argument('--data_setting', default='image_split', choices=['class_split', 'image_split'])
...
def main(args):
    ...
    cls_order = generate_cls_order(seed=args.seed_cls, shuffle=(args.data_setting == 'image_split'))

    if args.data_setting=='class_split':
        total_phase_num = args.num_of_phases
    elif args.data_setting=='image_split':
        total_phase_num = args.num_of_phases
    else:
